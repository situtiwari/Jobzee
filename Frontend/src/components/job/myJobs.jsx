...
const MyJobs = () => {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        const { data } = await axios.get("http://127.0.0.1:4000/api/v1/job/getMyJobs", { withCredentials: true });
  ...
  const handleEnableJob= (jobId) => {
    setEditingMode(jobId);
  }
  // Function For Disabling Editing Mode
  const handleDisableJob = (jobId) => {
    setEditingMode(null);
  }
  // Function For Editing Job
  const handleUpdateJob = async () => {
    const updatedJob = myjobs.find(job => job._id === jobId);
    await axios.put(`http://127.0.0.1:4000/api/v1/job/updateJob/${jobId}`, updatedJob, {
  ...
  const handleJobDelete = async (jobId) => {
    await axios.delete(`http://127.0.0.1:4000/api/v1/job/deleteJob/${jobId}`,
                        ...
                        <div className="edit_btn_wrapper">
                          {
                             ...
                             <button onClick={()=>handleUpdateJob(element._id)}
                              className='check_btn'> <FaCheck/></button>
                               <button onClick={()=>handleDisableJob()}
                              className='check_btn'> <RxCross2/></button></>) : 

                              (<button onClick={()=>handleEnableJob(element._id)}
